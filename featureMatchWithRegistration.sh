...
ImageMath $dim blob026.nii.gz BlobDetector grad026.nii.gz 400 grad048.nii.gz blob048.nii.gz 50
...
antsRegistration -r blobMatch.mat  --dimensionality 2 -f --float 1 --output [output,outputWarped.nii.gz,outputInverseWarped.nii.gz] --interpolation Linear --use-histogram-matching 1 --winsorize-image-intensities [0.005,0.995]  --transform affine[0.1] --convergence [100x70x50x10,1e-6,10] --shrink-factors 4x4x2x1 --smoothing-sigmas 3x2x1x0vox --metric mattes[$imgs,1,32,Random,0.5] --transform SyN[0.1,6,0.0] --convergence [100x70x50x10,1e-6,10] --shrink-factors 4x4x2x1 --smoothing-sigmas 3x2x1x0vox --metric cc[$imgs,1,4]  -x mask.nii.gz
x=048
...
antsApplyTransforms -d 2 $tx  \
   ...
   MeasureImageSimilarity 2 1 frames-026.nii.gz XXX.nii.gz 
MeasureImageSimilarity 2 1 frames-026.nii.gz XXX2.nii.gz
